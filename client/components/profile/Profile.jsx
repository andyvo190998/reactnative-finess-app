...
import { work2, work3, work4, work5, work6, work7 } from "../../assets/works.js";

import img from "../../assets/images/works/work2.jpg";
//https://www.mockplus.com/blog/post/profile-page-design
const workOutCard = ({ item }) => {
	return (
		<View key={item.id} className=" flex-1 mr-5 w-40 h-32 ">
			<ImageBackground source={item.img} resizeMode="cover" imageStyle={{ borderRadius: 14 }}>
...
const PostCards = ({ item }) => {
	return (
		<View key={item.id} className=" w-full flex flex-col bg-white rounded-2xl p-2 mb-2">
			...
			<View className="flex justify-center items-center h-full">
				<View className=" w-full flex justify-center items-center flex-row pt-20 px-5 mb-5">
					...
					<TouchableOpacity
						...
						className="bg-slate-50 rounded-lg h-9 w-9 flex justify-center items-center"
					>
				...
				<View className=" w-full flex flex-row justify-around mb-5">
					<View className="flex items-center">
						<Text style={{ fontFamily: "DMBold" }} className="text-slate-300 text-xl">
							108
						</Text>
				...
				<View className="w-full px-5  flex flex-col mb-5">
					<Text className="color-slate-300 mb-3 pl-1" style={{ fontFamily: "DMBold" }}>
						My Workouts
					</Text>
					<ScrollView
						showsVerticalScrollIndicator={false}
